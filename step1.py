...
def main():
    ...
    #TODO: make inspect_segmentation_after a separate script that can be run to inspect
    random.seed(999)
    np.random.seed(999)

    time_zero = time()
    tomo_dir = '/fs/pool/pool-engel/Spinach_project/folder_for_lorenz/180524/'
    ...
    val_tokens = None
    test_tokens = None
    # test_tokens = {'Tomo_0001': [('S1', 'M12')]}



    print(1, time() - time_zero)
    project_name = 'test_pipeline'
    project_directory = '/fs/pool/pool-engel/Lorenz/MemBrain_test_pipeline/'
    ...
    # normals_corrected_star = normal_voting_for_star(normals_star, os.path.join(project_directory, 'positions', 'normals_corrected'), npr=n_pr)
    ...
    dm = MemBrain_datamodule(out_star_name, 512, part_dists=['PSII'])
    heatmap_out_dir = os.path.join(project_directory, 'heatmaps')
    trainer = MemBrainer(box_range=6, dm=dm, project_dir=project_directory, star_file=out_star_name)
    trainer.train()
